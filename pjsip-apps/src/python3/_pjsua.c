...
static void cb_log_cb(int level, const char *data, int len)
{
	...
	param_data = PyUnicode_FromStringAndSize(data, len);
				      ...
				      pjsua_msg_data *msg_data)
{
	...
	if (contact_hdr) {
	    ...
	    param_contact = PyUnicode_FromStringAndSize(contact, len);
	} else {
					  ...
					  PyObject *pArgs)
{
    ...
    if (arg_reason && PyUnicode_Check(arg_reason)) {
...
static PyObject *py_pjsua_enum_snd_devs(PyObject *pSelf, PyObject *pArgs)
{
    ...
    for (i = 0; i < c; i++)  {
        ...
        obj->name = PyUnicode_FromString(info[i].name);
...
{    
    ...
    if (!PyUnicode_Check(pContent))
	return Py_BuildValue("i", PJ_EINVAL);

    content = PyString_ToPJ(pContent);

    if (PyUnicode_Check(pMimeType)) {
        mime_type = &tmp_mime_type;
        tmp_mime_type = PyString_ToPJ(pMimeType);
    } else {
	mime_type = NULL;   
...
{    	
    ...
    indent = PyUnicode_AsUTF8(pIndent);
...
static PyObject *py_pj_strerror(PyObject *pSelf, PyObject *pArgs)
{
    ...
    return PyUnicode_FromStringAndSize(err_msg, ret.slen);
}
...
static PyModuleDef testmod_module = {
    ...
    "_pjsua",                  // Module name to use with Python import statements
    "PJSUA-lib module for python",  // Module description
    0,
    py_pjsua_methods             // Structure that defines the methods of the module
};

PyMODINIT_FUNC PyInit__pjsua()
{
    PyObject* m = NULL;
#define ADD_CONSTANT(mod,name)	PyModule_AddIntConstant(mod,#name,name)
    ...
    m = PyModule_Create(&testmod_module);
