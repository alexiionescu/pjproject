...
PJ_INLINE(pj_str_t) PyString_ToPJ(const PyObject *obj)
{
	...
	str.ptr = PyUnicode_AsUTF8(obj);
	str.slen = PyUnicode_GET_LENGTH(obj);
    } else {
	str.ptr = NULL;
	str.slen = 0;
    }
					    ...
					    PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->realm = PyUnicode_FromString("");
        self->scheme = PyUnicode_FromString("");
        self->username = PyUnicode_FromString("");
	self->data_type = PJSIP_CRED_DATA_PLAIN_PASSWD;
        self->data = PyUnicode_FromString("");
    }
					      ...
					      PyObject *kwds)
{
    ...
    if (self != NULL) {
	self->turn_server = PyUnicode_FromString("");
	self->turn_realm = PyUnicode_FromString("");
	self->turn_username = PyUnicode_FromString("");
	self->turn_passwd = PyUnicode_FromString("");
    }
					    ...
					    const pjsua_media_config *cfg)
{
	...
	Py_XDECREF(obj->turn_realm);
	obj->turn_realm	= PyUnicode_FromString("");
	Py_XDECREF(obj->turn_username);
	obj->turn_username = PyUnicode_FromString("");
	obj->turn_passwd_type = 0;
	Py_XDECREF(obj->turn_passwd);
	obj->turn_passwd = PyUnicode_FromString("");
    }
}
				      ...
				      const pjsua_config *cfg)
{
	...
	obj->outbound_proxy = PyUnicode_FromString("");
				      ...
				      PyObj_pjsua_config *obj)
{
    ...
    cfg->thread_cnt	= obj->thread_cnt;
    if (PyUnicode_GET_LENGTH(obj->outbound_proxy) > 0) {
					...
					PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->user_agent = PyUnicode_FromString("");
        self->outbound_proxy = PyUnicode_FromString("");
	self->stun_domain = PyUnicode_FromString("");
	self->stun_host = PyUnicode_FromString("");
					         ...
					         PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->log_filename = PyUnicode_FromString("");
					   ...
					   PyObject *kwds)
{
    ...
    if (self != NULL) {
        ...
        self->content_type = PyUnicode_FromString("");
        self->msg_body = PyUnicode_FromString("");
    }
						   ...
						   PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->public_addr = PyUnicode_FromString("");
        self->bound_addr = PyUnicode_FromString("");
    }
						 ...
						 PyObject *kwds)
{
    ...
    if (self != NULL)
    {
        self->type_name = PyUnicode_FromString("");
        self->info = PyUnicode_FromString(""); 
        self->addr = PyUnicode_FromString("");
    }
					     ...
					     PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->id = PyUnicode_FromString("");
        self->reg_uri = PyUnicode_FromString("");
        self->force_contact = PyUnicode_FromString("");
	self->proxy = (PyListObject *)PyList_New(0);
	self->cred_info = (PyListObject *)PyList_New(0);
	self->auth_initial_algorithm = PyUnicode_FromString("");
	self->pidf_tuple_id = PyUnicode_FromString("");
	self->contact_params = PyUnicode_FromString("");
	self->contact_uri_params = PyUnicode_FromString("");
	self->ka_data = PyUnicode_FromString("");
    }
					   ...
					   PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->acc_uri = PyUnicode_FromString("");
	self->status_text = PyUnicode_FromString("");
	self->online_status_text = PyUnicode_FromString("");
    }
					     ...
					     PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->uri = PyUnicode_FromString("");
	self->contact = PyUnicode_FromString("");
	self->status_text = PyUnicode_FromString("");
	self->sub_term_reason = PyUnicode_FromString("");
    }
    return (PyObject *)self;
}
                                 ...
                                 PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->codec_id = PyUnicode_FromString("");
    }
    return (PyObject *)self;
}
                                    ...
                                    PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->local_info = PyUnicode_FromString("");
	self->local_contact = PyUnicode_FromString("");
	self->remote_info = PyUnicode_FromString("");
	self->remote_contact = PyUnicode_FromString("");
	self->call_id = PyUnicode_FromString("");
	self->state_text = PyUnicode_FromString("");
	self->last_status_text = PyUnicode_FromString("");
    }
    return (PyObject *)self;
}
...
static void PyObj_pjsip_rx_data_import(PyObj_pjsip_rx_data *obj, pjsip_rx_data *rx_data)
{
    ...
    obj->msg_info_buffer = PyUnicode_FromString(rx_data->msg_info.msg_buf);
    Py_XDECREF(obj->msg_info_info);
    obj->msg_info_info = PyUnicode_FromString(pjsip_rx_data_get_info(rx_data));
}
...
static PyObject * PyObj_pjsip_rx_data_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    if (self != NULL) {
        self->msg_info_buffer = PyUnicode_FromString("");
        self->msg_info_info   = PyUnicode_FromString("");
    }
