...
class AccountConfig:
    ...
    def build_config(self, domain, username, password, display="",
                     registrar="", proxy="", login="", rtp_transport_cfg = None):
        """
        Construct account config. If domain argument is specified, 
        a typical configuration will be built.
        ...
        if not login:
            login = username
        if login != "":
            self.auth_cred.append(AuthCred("*", login, password))
...
class Lib:
    ...
    def start(self, with_thread=True):
        ...
        if self._has_thread:
            _thread.start_new_thread(_worker_thread_main, (0,))
